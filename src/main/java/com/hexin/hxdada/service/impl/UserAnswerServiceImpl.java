package com.hexin.hxdada.service.impl;
...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
    ...
    public void validUserAnswer(UserAnswer userAnswer, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(choices), ErrorCode.PARAMS_ERROR, "答题选项不能为空");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "appId 非法");
            ThrowUtils.throwIf(resultId == null || resultId <= 0, ErrorCode.PARAMS_ERROR, "resultId 非法");
            ThrowUtils.throwIf(resultScore == null || resultScore <= 0, ErrorCode.PARAMS_ERROR, "用户答题得分异常");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (appId!=null) {
        ...
        if (resultId!=null) {
            ScoringResult scoringResult = scoringResultService.getById(resultId);
            ThrowUtils.throwIf(scoringResult==null, ErrorCode.PARAMS_ERROR, "resultId 非法");
        }
    }
    ...
    public QueryWrapper<UserAnswer> getQueryWrapper(UserAnswerQueryRequest userAnswerQueryRequest) {
        ...
        queryWrapper.eq(AppTypeEnum.getEnumByValue(appType)!=null, "resultScore", resultScore);
        queryWrapper.eq(ScoringStrategyEnum.getEnumByValue(scoringStrategy)!=null, "resultScore", resultScore);
